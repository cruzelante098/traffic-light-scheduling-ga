...
export function run(population: number, crossovers: "OnePointCrossover" | "UniformCrossover", inputFile: string) {
  for (let i = 0; i < 1; i++) {
    ...
    const node = spawn("\"C:/Program Files/nodejs/node.exe\"", [
      ...
      `--save "${outputFolder}/${fileFolder}/${crossovers}_Pop${population}_${filename}_${i + 1}"`,
      `--savegenotype`,
    ], {
      cwd: process.cwd(),
      env: process.env,
...
let instancesFolder = "C:/Users/Francisco Cruz/Repositorios/traffic-light-scheduling-ga/assets/instances";
