...
sub leveneTest{
	...
	$scriptR = "data1 = scan(\\\"$file1\\\");
data2 = scan(\\\"$file2\\\");
install.packages(\\\"lawstat\\\",  repos = \\\"http://cran.us.r-project.org\\\", INSTALL_opts = '--no-lock');
library(\\\"lawstat\\\");\n";
	$scriptR .= "grupos<-c(";
	for my $i (1..$repeticiones){
		$scriptR .= "1, ";		
	...
	$resultado = `echo "$scriptR" | R --no-save --silent | grep "\\\[1\\\]"`;
	if ($resultado =~ /\[1\] (.*)/){
		return $1;
	} else {
    print($resultado);
...
printf("$mean1 $mean2 $median1 $median2 $pValue\n");
